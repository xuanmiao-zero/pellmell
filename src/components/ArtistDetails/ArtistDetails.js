...
class ArtistDetails extends React.PureComponent {
  ...
  ATweenNum = (obj, count, duration, attr, fx, fn) => {
    ...
    obj.timer = setInterval(function () {
      ...
      obj[attr] = Tween[fx](t, begin, count, duration);
      //是否有fn参数
      if (t >= duration && fn) fn();
    }, 20);
  }
  ...
  downScroll = () => {
    if (getScrollTop() < document.documentElement.clientHeight - 1) {
      this.ATweenNum(document.documentElement, document.documentElement.clientHeight - getScrollTop(), 400, 'scrollTop', 'easeOut')
    }
  }
  upScroll = () => {
    if (getScrollTop() <= getWindowHeight()) {
      this.ATweenNum(document.documentElement, -document.documentElement.clientHeight, 400, 'scrollTop', 'easeOut')
    }
  }
  ...
  componentDidMount() {
    ...
    scroll(option)
    console.log(this.props.actions.getWaterfallFlowData('wallhaven'))
  }
